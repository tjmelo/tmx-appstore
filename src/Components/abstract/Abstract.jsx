...
export const Abstract = () => {
  ...
  const realTotal = subtotal.map(
    (el) => el.optionsProduct.realTotal
  );
  const baseCalc = subtotal.map(
    (el) => el.optionsProduct.baseCalc
  );
  const discountTotal = subtotal.map(
    (el) => el.optionsProduct.discountTotal
  );

  const sumTotal = baseCalc
    .reduce((sum, val) => sum + val, 0)
    .toFixed(2);
  const sumTotalDiscount = discountTotal
    .reduce((sum, val) => sum + val, 0)
    .toFixed(2);
  const sumDiscount = realTotal
    .reduce((sum, val) => sum + val, 0)
    .toFixed(2);
